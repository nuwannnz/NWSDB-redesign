...
import { Button, View, Text, StyleSheet, Modal, Alert, Picker } from 'react-native';
...
export default function UserComplaintScreen() {
  ...
  const [accounts, setAccounts] = useState([
    ...
    const style = StyleSheet.create({
      ...
      const [selectedAccount, setSelectedAccount] = useState({account: '', personName: '', address: ''});

      const clickAccount = (account: string, personName: string, address: string) => {
        setSelectedAccount({account: account, personName: personName, address: address});
        setModalOpen(true);
      }
        ...
        return (
           ...
           <View style={style.container}
            ...
            <FlatList
              data={accounts}
              renderItem={({ item }) => (
                <Button  title={item.name+' - '+  item.number} onPress={() => clickAccount(item.number, item.personName, item.address)}> </Button>
          ...
          <View style={styles.modalView}>
            <View style={style.statusBox}>
              <Text style={styles.modalText}>Account No : {selectedAccount.account}</Text>
              <Text style={styles.modalText}>Persona Name : {selectedAccount.personName}</Text>
              <Text style={styles.modalText}>Address : {selectedAccount.address}</Text>
            </View>
              <Text style={styles.modalText}>Complaint category  : </Text>
               ...
               <Picker.Item label="- select category -" value="0" />
                ...
                <View
                      ...
                      <Text></Text>
                    <Button title="Submit" color="green" onPress={() => setModalOpen(false)}></Button>
